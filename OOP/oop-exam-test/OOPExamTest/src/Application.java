...
public class Application {
        ...
        while (command.getType() != CommandType.EXIT) {
            ...
            switch (command.getType()) {
                ...
                case FILTER_PRODUCT_BY_NAME:
                    handleFilterByName(command.getArgs());
                    break;
    ...
    private void handleListProductsByCategory(List<String> args) {
        Category category = categoriesService.getOrCreate(args.get(0));
        String output = productsService.getProductsByCategory(category).stream()
    ...
    private void handleListProducts() {
        String output = productsService.listAllProducts().stream()
                .map(product -> String.format("%s (%d)", product.getName(), product.getId()))
                .collect(Collectors.joining(", "));
    ...
    private void handleAddProduct(List<String> args) {
        ...
        List<Category> categories = args.subList(2, args.size()).stream().map(categoriesService::getOrCreate)
    ...
    private void handleFilterByName(List<String> args) {
        String filter = args.get(0);
        List<Product> newProducts = productsService.search(p -> p.getName().toLowerCase().contains(filter));
        newProducts.forEach(n -> System.out.println(n.getName()));
    }
