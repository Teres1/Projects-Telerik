...
public class CreditCard extends BankCard implements BillsPayable, CardValidityCheckable {
    ...
    public CreditCard(String name, String idOrBulstat, int incomesMonthly, int annualMaintenance, double withdrawTax, double creditLimit) {
    ...
    public void billsPayable(double sumNeededToPayBill) {
        if (sumNeededToPayBill < getCreditLimit()) {
    ...
    public void cardValidityCheck(int validityYear, int validityMonth, int validityDay) {
        ...
        m = cal.get(Calendar.MONTH);// current month
        d = cal.get(Calendar.DAY_OF_MONTH);//current day
        cal.set(validityYear, validityMonth, validityDay);// here card validity year,month,day
